...
function drawAreaChart(option) {
  ...
  d3.selectAll(".container #x-axis g.tick")
    .append("line")
  ...
  d3.selectAll(".container #y-axis g.tick")
    .append("line")
    .attr("class", "grid-line")
    .attr("x1", 0)
...
// format the data
temperatureDate = temperatureDate.map(d => {
  return {
    date: new Date(d[0]),
    value: +d[1]
  };
...
drawAreaChart({
  ...
  Ylabel: "value",
  minYvalue: 90,
  maxYvalue: 105, //d3.max(data, d => d.value),
  highlightHline1: 95,
  highlightHline2: 103
});
